...
type OutputDetails struct {
	...
	FundingAddressTree *FundingTree // required for funding address claims
	FundingAddressId   string       // when set, triggers direct funding address claim flow
...
func ConstructTransaction(network *Network, currency Currency, outputs []OutputDetails, fee Fee, boltzApi *Api) (Transaction, Results, error) {
	...
	for i, output := range outputs {
		err = func() error {
			...
			// Handle direct funding address claim (not through a swap)
			if output.FundingAddressId != "" {
				return handleFundingAddressClaim(network, currency, transaction, outputs, i, boltzApi)
			}
				...
				signature, err = func() (*PartialSignature, error) {
					...
					if details.FundingAddressId != "" {
						if output.FundingAddressTree == nil {
							return nil, fmt.Errorf("FundingAddressTree is required for cooperatively claiming chain swap with funding address")
						}
...
// handleFundingAddressClaim handles cooperative signing for direct funding address claims.
func handleFundingAddressClaim(
	network *Network,
	currency Currency,
	...
	if output.FundingAddressTree == nil {
		return errors.New("FundingAddressTree is required for claiming funding address")
	}
	...
	claimRequest := &FundingAddressClaimRequest{
		PubNonce:        ourNonce[:],
		TransactionHash: txHash,
	}

	boltzSignature, err := boltzApi.ClaimFundingAddress(output.FundingAddressId, claimRequest)
	if err != nil {
		return fmt.Errorf("could not get partial signature from Boltz: %w", err)
	}
