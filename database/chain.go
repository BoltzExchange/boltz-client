...
const insertChainSwap = `
		...
		(id, fromCurrency, toCurrency, state, error, status, acceptZeroConf, preimage, isAuto, serviceFee, serviceFeePercent, onchainFee, createdAt, entity)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`
...
const insertChainSwapData = `
		...
		(id, currency, privateKey, theirPublicKey, blindingKey, tree, amount, timeoutBlockheight, lockupTransactionId, transactionId, wallet, address, lockupAddress)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`
...
func (database *Database) UpdateChainSwap(swap ChainSwap) error {
	_, err := database.Exec(`
		...
		entity = ?
		WHERE id = ?
	`, swap.FromCurrency, swap.ToCurrency, swap.State, swap.Error, swap.Status, swap.Preimage, swap.BlindingKey, swap.IsAuto, swap.ServiceFee, swap.ServiceFeePercent, swap.OnchainFee, swap.CreatedAt, swap.Entity, swap.ID)
	return err
}
...
func (database *Database) UpdateChainSwapData(swapData ChainSwapData) error {
	_, err := database.Exec(`
		...
		address = ?
		WHERE id = ?
	`, swapData.Currency, swapData.PrivateKey, swapData.TheirPublicKey, swapData.Tree, swapData.Amount, swapData.TimeoutBlockHeight, swapData.LockupTransactionID, swapData.ClaimTransactionID, swapData.RefundTransactionID, swapData.Wallet, swapData.Address, swapData.ID)
	return err
}
*/
