...
func listFundingAddresses(ctx *cli.Context) error {
	...
	tbl := table.New("ID", "Currency", "Address", "Status", "Timeout Height", "Created At")
	tbl.WithHeaderFormatter(headerFmt).WithFirstColumnFormatter(columnFmt)

	for _, fa := range response.FundingAddresses {
		tbl.AddRow(fa.Id, fa.Currency, fa.Address, fa.Status, fa.TimeoutBlockHeight, parseDate(fa.CreatedAt))
	}
...
var claimFundingAddressCommand = &cli.Command{
	Name:      "claimfunding",
	Category:  "Funding",
	Usage:     "Claim a funding address to an address or wallet",
	ArgsUsage: "<funding_address_id> <address|wallet>",
	Description: "Claims a funded funding address by cooperatively signing with Boltz.\n" +
	...
	Action: claimFundingAddress,
	Flags:  []cli.Flag{jsonFlag},
}

func claimFundingAddress(ctx *cli.Context) error {
	client := getClient(ctx)
	...
	request := &boltzrpc.ClaimFundingAddressRequest{
		FundingAddressId: fundingAddressId,
	}
	...
	if err == nil {
		request.Destination = &boltzrpc.ClaimFundingAddressRequest_WalletId{WalletId: *walletId}
	} else {
		request.Destination = &boltzrpc.ClaimFundingAddressRequest_Address{Address: destination}
	}

	response, err := client.ClaimFundingAddress(request)
	if err != nil {
		return err
	}
	...
	fmt.Println("Funding address claimed successfully!")
	fmt.Printf("  Funding Address ID: %s\n", fundingAddressId)
