...
func createSwap(ctx *cli.Context) error {
	...
	feeEstimate, err := client.GetFeeEstimation(boltzrpc.SwapType_SUBMARINE, pair, uint64(amount))
	if err != nil {
		return err
	}

	if !json {
		printFees(feeEstimate)
	...
	if externalPay {
		printDeposit(amount, swap.Address, swap.TimeoutHours, uint64(swap.TimeoutBlockHeight), feeEstimate.Limits)
		fmt.Println()
	}
...
func printFees(estimate *boltzrpc.FeeEstimation) {
	fmt.Println("The fees for this service are:")
	fmt.Printf("  - Service fee: %.1f%%\n", estimate.Fees.Percentage)
	fmt.Printf("  - Miner fee: %s\n", utils.Satoshis(estimate.Fees.MinerFees))
	fmt.Println()
	if estimate.TotalFee > 0 {
		fmt.Printf("Total fee for %s: %s\n", utils.Satoshis(estimate.AdjustedAmount), utils.Satoshis(estimate.TotalFee))
	}
}
...
func createChainSwap(ctx *cli.Context) error {
	...
	feeEstimate, err := client.GetFeeEstimation(boltzrpc.SwapType_CHAIN, pair, amount)
	if err != nil {
		return err
	}

	request.Amount = feeEstimate.AdjustedAmount
	...
	if externalPay {
		...
		printDeposit(amount, swap.FromData.LockupAddress, float32(timeoutHours), uint64(timeout), feeEstimate.Limits)
		fmt.Println()
	}
...
func createReverseSwap(ctx *cli.Context) error {
	...
	if !json {
		feeEstimate, err := client.GetFeeEstimation(boltzrpc.SwapType_REVERSE, pair, amount)
		if err != nil {
			return err
		}
