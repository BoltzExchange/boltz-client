...
func (cfg *Config) Init() (*BlockchainBackend, error) {
	var err error
...
func (w *Wallet) syncLoop(ctx context.Context) {
	for {
		jitter := time.Duration(float64(w.backend.cfg.SyncInterval) * (0.75 + rand.Float64()*0.5))
		time.Sleep(jitter)
		select {
		case <-ctx.Done():
			w.syncWait.Done()
...
func (w *Wallet) autoConsolidate() error {
	...
	if len(utxos) >= int(w.backend.cfg.ConsolidationThreshold) {
		logger.Debugf("Auto consolidating wallet %s", w.info)
