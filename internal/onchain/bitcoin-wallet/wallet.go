...
func NewBackend(cfg Config) (*Backend, error) {
	...
	return &Backend{backend, cfg.TxProvider, cfg.FeeProvider}, nil
}
...
func (backend *Backend) NewWallet(credentials *onchain.WalletCredentials) (onchain.Wallet, error) {
	...
	if credentials.CoreDescriptor != "" {
		creds = bdk.WalletCredentialsCoreDescriptor{Field0: credentials.CoreDescriptor}
	}
	if credentials.Mnemonic != "" {
		creds = bdk.WalletCredentialsMnemonic{Field0: credentials.Mnemonic}
		info.Readonly = false
	}
