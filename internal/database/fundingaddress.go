...
INSERT INTO fundingAddresses (id, currency, address, amount, timeoutBlockheight, boltzPublicKey, privateKey, blindingKey, status, lockupTransactionId, swapId, createdAt, tenantId)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`
...
func (database *Database) SetFundingAddressLockupTransaction(fa *FundingAddress, lockupTransactionId string, amount uint64) error {
	fa.LockupTransactionId = lockupTransactionId
	fa.Amount = amount

	_, err := database.Exec(
		"UPDATE fundingAddresses SET lockupTransactionId = ?, amount = ? WHERE id = ?",
		lockupTransactionId,
		amount,
		fa.Id,
	)
