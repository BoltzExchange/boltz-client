...
func (server *routedBoltzServer) importWallet(ctx context.Context, credentials *wallet.Credentials, password string) error {
	...
	return server.database.RunTx(func(tx *database.Transaction) error {
		if err := tx.CreateWallet(&database.Wallet{Credentials: credentials}); err != nil {
			return err
		}
		...
		imported, err := loginWallet(credentials)
		if err != nil {
...
func (server *routedBoltzServer) encryptWalletCredentials(tx *database.Transaction, password string, credentials []*wallet.Credentials) (err error) {
	for _, creds := range credentials {
		if password != "" {
...
func loginWallet(credentials *wallet.Credentials) (onchain.Wallet, error) {
	if credentials.Currency == boltz.CurrencyLiquid {
		return liquid_wallet.NewWallet(credentials)
	}
