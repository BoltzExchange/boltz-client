...
func TestFundingAddress(t *testing.T) {
	...
	t.Run("ClaimFundingAddress", func(t *testing.T) {
		t.Run("ClaimToAddress", func(t *testing.T) {
			for _, tc := range currencies {
				t.Run(tc.desc, func(t *testing.T) {
					fundingAddress, err := boltzClient.CreateFundingAddress(&boltzrpc.CreateFundingAddressRequest{
						Currency: tc.currency,
					...
					claimResp, err := boltzClient.ClaimFundingAddress(&boltzrpc.ClaimFundingAddressRequest{
						FundingAddressId: fundingAddress.Id,
						Destination:      &boltzrpc.ClaimFundingAddressRequest_Address{Address: destAddress},
					})
					require.NoError(t, err)
		...
		t.Run("ClaimToWallet", func(t *testing.T) {
			for _, tc := range currencies {
				t.Run(tc.desc, func(t *testing.T) {
					...
					claimResp, err := boltzClient.ClaimFundingAddress(&boltzrpc.ClaimFundingAddressRequest{
						FundingAddressId: fundingAddress.Id,
						Destination:      &boltzrpc.ClaimFundingAddressRequest_WalletId{WalletId: wallet.Id},
					})
					require.NoError(t, err)
					require.NotEmpty(t, claimResp.TransactionId)
		...
		t.Run("Errors", func(t *testing.T) {
			t.Run("NotFunded", func(t *testing.T) {
				...
				_, err = boltzClient.ClaimFundingAddress(&boltzrpc.ClaimFundingAddressRequest{
					FundingAddressId: fundingAddress.Id,
					Destination:      &boltzrpc.ClaimFundingAddressRequest_Address{Address: destAddress},
				})
				require.Error(t, err)
			...
			t.Run("Destination", func(t *testing.T) {
				...
				_, err = boltzClient.ClaimFundingAddress(&boltzrpc.ClaimFundingAddressRequest{
					FundingAddressId: fundingAddress.Id,
					Destination:      &boltzrpc.ClaimFundingAddressRequest_WalletId{WalletId: invalidWalletId},
				})
				require.Error(t, err)
				requireCode(t, err, codes.NotFound)
			})
		...
		t.Run("TenantPermissionDenied", func(t *testing.T) {
			_, write, _ := createTenant(t, boltzClient, "claim-funding-tenant")
			...
			destAddress := test.BtcCli("getnewaddress")
			_, err = tenantClient.ClaimFundingAddress(&boltzrpc.ClaimFundingAddressRequest{
				FundingAddressId: adminFunding.Id,
				Destination:      &boltzrpc.ClaimFundingAddressRequest_Address{Address: destAddress},
			})
			require.Error(t, err)
